...
/*
 * ADC10CTL0 C’est le registre de contrôle principal de l’ADC10.
 * ENC (Enable Conversion)
   Autorise l’ADC à effectuer une conversion
   Doit être à 1 pour lancer l’ADC
 *
 * ADC10SC (Start Conversion) Démarre immédiatement la conversion analogique → numérique
 *
 * ADC10CTL1 :Deuxième registre de contrôle de l’ADC

🔹 ADC10BUSY :Bit indicateur

= 1 → conversion en cours
= 0 → conversion terminée

ADC10MEM: Registre mémoire de l’ADC,Contient le résultat final de la conversion.
 */
...

{
     ...
     *
     * ADC10CTL1:Registre de contrôle n°1 de l’ADC10.
             🔹 INCH_2:Signifie Input Channel 2 ,Correspond à la broche A2 → P1.2
     *
     * SREF_0 → Source de référence
      SREF_0 = référence Vcc / GND
     *
     * ADC10SHT_3 → Temps d’échantillonnage ,Définit le temps pendant lequel l’ADC “écoute” le signal
      ADC10SHT_3 = 64 cycles ADC
     * ADC10ON → Activation de l’ADC,Met l’ADC sous tension
     * ADC10AE0:Registre d’activation analogique
     *
     * */
     ...
     * TACLR → Remise à zéro
       Réinitialise le compteur du timer
       Efface les anciens réglages internes
     *
     * */
    TA0CTL = TASSEL_2 | ID_3 | MC_1 | TACLR; // SMCLK /8, mode UP
    TA0CCR0 = 125000 - 1;                  // 1 MHz / 8 = 125 kHz → 1 s
    TA0CCTL0 = CCIE;                       // interruption activée
    ...
    P1DIR &= ~(CAPTEUR_GAUCHE | CAPTEUR_DROITE | CAPTEUR_OBS); // entrées
...

    {
        ...
        {
            tourner_gauche();   // corrige à gauche
        }
        else if (!gauche && droite)
        {
            tourner_droite();   // corrige à droite
        }
        else
        {
